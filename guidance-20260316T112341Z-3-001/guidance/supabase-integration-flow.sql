-- Guidance Integration Flow table for cross-department data exchange
-- Run this after 09-guidance.sql
...
CREATE INDEX IF NOT EXISTS idx_guidance_integration_direction_status
  ON guidance.integration_flows(direction, status);

CREATE INDEX IF NOT EXISTS idx_guidance_integration_departments_active
  ON guidance.integration_departments(is_active);

CREATE INDEX IF NOT EXISTS idx_guidance_integration_routes_source_target
  ON guidance.integration_routes(source_department_key, target_department_key);

CREATE INDEX IF NOT EXISTS idx_guidance_integration_routes_event_code
  ON guidance.integration_routes(event_code);

CREATE INDEX IF NOT EXISTS idx_guidance_integration_created_at
  ON guidance.integration_flows(created_at DESC);

CREATE INDEX IF NOT EXISTS idx_guidance_integration_route_key
  ON guidance.integration_flows(route_key);

CREATE INDEX IF NOT EXISTS idx_guidance_integration_event_code
  ON guidance.integration_flows(event_code);

CREATE INDEX IF NOT EXISTS idx_guidance_integration_correlation_id
  ON guidance.integration_flows(correlation_id);

CREATE INDEX IF NOT EXISTS idx_guidance_integration_source_target
  ON guidance.integration_flows(source_department, target_department);
